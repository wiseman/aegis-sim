...
import { RADAR_RANGE_NM } from '../constants';
...
const RadarDisplay: React.FC<RadarDisplayProps> = ({ 
  ...
  const size = 600;
  const center = size / 2;
  const scale = (size / 2) / RADAR_RANGE_NM; // pixels per NM
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center bg-radar-black overflow-hidden shadow-inner shadow-black">
       ...
       <svg width={size} height={size} className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 overflow-visible">
          ...
          {/* Range Rings */}
          <circle cx={center} cy={center} r={scale * 20} className="stroke-gray-800 fill-none stroke-1" />
          <circle cx={center} cy={center} r={scale * 40} className="stroke-gray-800 fill-none stroke-1" />
          <circle cx={center} cy={center} r={scale * 60} className="stroke-gray-800 fill-none stroke-1" />
          <circle cx={center} cy={center} r={scale * 80} className="stroke-gray-700 fill-none stroke-2" />
